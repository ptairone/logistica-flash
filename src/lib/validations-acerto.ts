...
export const acertoSchema = z.object({
  ...
  total_bonificacoes: z.number().min(0).optional(),
  total_penalidades: z.number().min(0).optional(),
  total_debitos_descontados: z.number().min(0).optional(),
  total_ajustes_admin: z.number().min(0).optional(),
  total_pagar: z.number().optional(),
  status: z.enum(['aberto', 'fechado', 'pago']),
  etapa_atual: z.string().optional(),
  comprovantes_validados: z.boolean().optional(),
